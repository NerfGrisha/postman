...
def showdata():
    ...
    if spesifikasi_id:
        # If spesifikasi_id is provided, fetch data for that specific ID
        ...
        # If spesifikasi_id is not provided, fetch all data
        sql = "SELECT * FROM spesifikasi"
        cursor.execute(sql)
        all_data = cursor.fetchall()
        respond = jsonify(all_data) if all_data else jsonify('Data tidak ditemukan!')
